...
Мы будет при помощи функции `mutate_geocode()` из библиотеки  `ggmap  ` к существующему фрейму данных добавлять столбцы с координатами 
...
Находим наши координаты (Метро университет). Под  Windows функция `geocode()` видимо отправляет запрос к Google не в той кодировке -- пришлось транслитерировать адрес латинскими буквами.
