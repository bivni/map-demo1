...
_Примечание._ Под  Windows функция `geocode()` видимо отправляет запрос к Google не в той кодировке -- пришлось транслитерировать адрес латинскими буквами.

```{r,eval=T}
 ourAddress <- "Moskva, stancia metro Universitet" 
...
* Воспользоваться фукцией для перевода в латиницу `stri_trans_general("Москва, Станция метро Университет", "cyrillic-latin; nfd; [:nonspacing mark:] remove; nfc")`

* Воспользоваться фунцией для перевода из одной русской в другую русскую кодировку `geocode(iconv("станция метро Университет",from = "1251",to="UTF-8"))`
