...
Для удобства добавим в фрейм данных geotagsMGU столбец name, дублирующий номера строк. 
...
Установим масштаб (zoom) карты и укажем 1 точку в качестве центра карты при помощи функции `setView()`
...
Фрейм данных по умолчанию указан еще в функции leaflet(), а в остальных  функциях пакета leaflet мы можем ссылаться на столбцы этого dataframe в форме `~имяСтолбца` .