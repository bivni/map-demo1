...
Текст внутри всплывающих окон (popup) объектов верстается как таблица по правилам языка HTML. В первом примере использовались теги `<b>` и `</b>` для выделения текста полужирным (bold) шрифтом, а тег `<br>` для разделения текста на строки внутри таблицы. Если в названии объекта есть специально трактуемые html-символы, например, символы "<" и ">" -- (greater than & less than)  в строке "<ДАС>", то строку нужно подвергнуть "санитарной обработке" функцией `htmlEscape()` и заменить тем самым спецсимволы на их html-коды `&gt` и `&lt`, чтобы они не ломали структуру web-страницы.
```{r htmlEscape-ex}
htmlEscape("<ДАС>")
```